...
def compute_confidence(
    ...
    """
    Calcule un indicateur pédagogique de confiance.

    Il combine :
    - la performance globale du modèle ;
    - la proximité des valeurs saisies par rapport aux plages
      observées dans les données d'entraînement.

    Ce score n'est pas un intervalle de confiance statistique.
    """
...
encoder, model, scaler, artifact_errors = load_artifacts()
...
def predict_one(
    ...
    if encoder is None:
        raise ValueError("L'encodeur n'est pas disponible.")
...
if model is None or encoder is None or scaler is None:
    ...
    st.error(
        "⚠️ Les fichiers du modèle sont introuvables "
        "ou n'ont pas pu être chargés."
    )
...
<div class="hero">
    ...
    <p>
        Plateforme de Machine Learning pour anticiper
        la réussite académique et cibler
...
"""
    ...
    st.caption(
        "Combine la précision globale du modèle "
        "(R²) et la plausibilité des valeurs "
        "saisies par rapport aux données "
        "d'entraînement. Ce n'est pas un "
        "intervalle de confiance statistique classique."
    )
