...
Modèle : Ridge Regression (scikit-learn) entraîné sur 11 000 observations.
Métriques réelles de validation : R² = 0.9892 | RMSE = 2.04
"""
...
# =========================================================================
# CONSTANTES — dérivées des statistiques du dataset d'entraînement
# =========================================================================
FEATURE_RANGES = {
    "Heures_etude": {
        "min": 1,
...
@st.cache_resource
def load_artifacts():
    """Charge le modèle, l'encodeur et le scaler."""
    if not (
...
def compute_confidence(
    ...
    confidence = base * (
        0.55 + 0.45 * plausibility
    )

    return round(
        min(99.5, max(35.0, confidence)),
        1
    )
...
def predict_one(
    ...
    """Effectue une prédiction pour un étudiant."""

    if activite not in encoder.classes_:
        raise ValueError(
    ...
    # Application du scaler utilisé pendant l'entraînement
    ...
    # Limitation de la prédiction entre 10 et 100
    y_pred = round(
        min(
            TARGET_MAX,
            max(TARGET_MIN, y_pred_raw)
        ),
        2
    )
...
# =========================================================================
def add_to_history(row: dict):
    row["Horodatage"] = datetime.now().strftime(
        "%Y-%m-%d %H:%M:%S"
    )
...
<div class="hero">
    ...
    <p>
        Plateforme de Machine Learning pour anticiper la réussite académique
        et cibler l'accompagnement pédagogique — Régression Ridge, R² = 98.9%
    </p>
...
if model is None or encoder is None or scaler is None:
    st.error(
        f"⚠️ Fichiers modèle introuvables. Placez "
        f"**{MODEL_PATH}**, **{ENCODER_PATH}** et "
        f"**{SCALER_PATH}** dans le même dossier que app.py, "
        f"puis relancez l'application."
    )
...
with st.sidebar:
    ...
    - **R² (validation)** : `{MODEL_R2:.4f}`
    - **RMSE (validation)** : `{MODEL_RMSE:.2f} pts`
    - **Données d'entraînement** : 11 000 étudiants
    - **Variable cible** : Indice de performance (10–100)
    """
    )
    ...
    st.markdown(
        "### ℹ️ À propos du score de confiance"
    )
    ...
    with col_result:
        ...
        if "last_result" in st.session_state:
            ...
            <div class="result-card"
                ...
                <div class="result-sub">
                    Intervalle approx. (95%) :
                    {interval_min:.1f} – {interval_max:.1f}
                    &nbsp;|&nbsp;
                    Confiance : {r['Confiance']}%
                </div>
            ...
            if debug is not None:
                ...
                with st.expander(
                    ...
                    st.write(
                        f"**Prédiction brute avant clipping [10, 100]** : "
                        f"`{raw}`"
                    )

                    st.write(
                        f"**Colonnes utilisées** : "
                        f"`{debug['colonnes_utilisees']}`"
                    )

                    st.write(
                        f"**Valeurs brutes envoyées** : "
                        f"`{debug['valeurs_brutes']}`"
                    )

                    if raw <= TARGET_MIN:

                        st.warning(
                            "La valeur brute est déjà ≤ 10 avant clipping. "
                            "Vérifiez que l'ordre des variables et le scaler "
                            "utilisés dans l'application correspondent exactement "
            ...
            st.info(
                ...
                "**Lancer la prédiction** pour voir le résultat."
            )
    ...
    if fichier is not None:
        ...
        try:
            ...
            if manquantes:
                ...
                st.success(
                    f"✅ {len(df_csv)} prédictions effectuées avec succès."
                )
...
with tab3:
    ...
    if not st.session_state.history:
        ...
        st.info(
            "Aucune prédiction pour le moment. Utilisez l'onglet "
            "**Prédiction individuelle** ou **CSV**."
        )
        ...
        hist_df["Profil"] = hist_df.apply(
            lambda r: [
                ...
                r["Sujets_entrainement_pratiques"]
                / FEATURE_RANGES[
                    "Sujets_entrainement_pratiques"
                ]["max"],
            ],
